-- Phase 3: 거래 및 전송 시스템 데이터베이스 스키마 (수정된 버전)
-- Supabase SQL Editor에서 실행하세요

-- 1. 거래 기록 테이블
CREATE TABLE IF NOT EXISTS transactions (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  teacher_id UUID, -- teachers 테이블의 id 참조 (외래키는 나중에 추가)
  
  -- 거래 당사자
  from_student_id UUID, -- students 테이블의 id 참조
  to_student_id UUID, -- students 테이블의 id 참조
...
CREATE TABLE IF NOT EXISTS economic_entities (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  teacher_id UUID, -- teachers 테이블의 id 참조
...
CREATE TABLE IF NOT EXISTS loans (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  teacher_id UUID, -- teachers 테이블의 id 참조
  student_id UUID, -- students 테이블의 id 참조
